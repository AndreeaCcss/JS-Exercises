...
/*
  ...
  The manager has all the same properties as an Employee. Copy the Employee class 
  and rename it Manager.
...
/*
  ...
  Progressive managers have all the same properties as managers. 
  Copy the Manager class and rename
  it to ProgressiveManager. Add the following additional properties:
    - title - default 'Not a manager'
    - bonus - default 0

  When employees are hired or fired, the manager's title should be updated
...
/*
  ...
  A Machine has the following methods:
    ...
    - reboot
        - This function returns an anonymous function that is called when the machine 
        is done rebooting
...
class Machine {
  constructor(){
    ...
    // this.makeWidgets = (number) => {
    // this.widgets_made_count += number;
    // for(let i = 0; i < this.widgets_made_count; i++){if(i % 50 === 0) {this.wear_and_tear_count ++}}};

    this.makeWidgets = (number) => {
      this.widgets_made_count += number;
      const newCount = Math.trunc(this.widgets_made_count / 50);
      this.wear_and_tear_count = Math.max(newCount, 1);
    };
