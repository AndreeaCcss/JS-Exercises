...
/*
  ...
  there are more accidents this year than there actually are.
    1. Write a function called 'removeDuplicates' that will remove all duplicate 
    entries in the workplaceAccidents array.
...
function removeDuplicates(){
  // let unique = [...new Set(workplaceAccidents)]
  // return unique
  return workplaceAccidents = workplaceAccidents.filter((element, index, array) => 
...
function looper () {
  for(i = 0; i < numsArr.length; i++){
    for(m = 0; m < numsArr[i].length; m++){
      if(numsArr[i][m] % 2 === 0){
        console.log(numsArr[i][m])
        numsArr[i][m] = "even";
      } else {
        console.log(numsArr[i][m])
        numsArr[i][m] = "odd";
