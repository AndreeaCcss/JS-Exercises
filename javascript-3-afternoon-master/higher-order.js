...
//Code Here
let evenNumbers  = mixedNumbers.filter(word => word % 2 == 0)
...
let orderTotals = orders.map(order => {
  let totalPrice = order.price * (1 + order.tax);
  return totalPrice;
});
...
let bobsTotal = purchases.filter(owner => owner.owner == "Bob");
bobsTotal = bobsTotal.reduce((acc, val) => {
  // console.log(acc)
